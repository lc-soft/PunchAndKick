...
L"    需要为游戏角色添加使用武器的动作，弄简单点，就将武器放在游戏角色的手上，原动作保持不变，只添加使用武器进行攻击的动作。弄复杂点的话，根据武器的不同，改变游戏角色的动作，例如:剑和枪，游戏角色装备它们时的动作姿势就像剑圣与赵信。可将使用武器进行攻击的动作分为捅、砍、扔这几种，感觉需要将手臂、身体进行分层，分为两个独立的图层，武器就是夹在这两个图层中间的图层。至于道具，按功能分类的话，主要分为加血、加属性、无敌化这几种功能。\n"
L"\n"
